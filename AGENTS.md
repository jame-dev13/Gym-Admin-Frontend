...
This directory (`.git`, `package.json`, `src/`, etc. all at this level) **is** the git repo root. From now on run all commands and reference paths from here.
...
- **`@/*` path alias is configured in both places.** `tsconfig.app.json` declares `"@/*": ["./src/*"]`, and `vite.config.ts` sets `resolve.alias` mapping `"@"` to `./src` (via `path.resolve(import.meta.dirname, "./src")`) — so `@/` imports are safe to use.
...
- **`pnpm-workspace.yaml` allows `msw` build scripts** (`allowBuilds: msw: true`). Leave it as is; only revisit if pnpm complains about build scripts or `msw`.
...
4. **Push, then open the PR.** `git push -u origin <branch>` (set upstream on first push), then create the PR via the GitHub MCP server (`github_create_pull_request`, owner `jame-dev13`, repo `Gym-Admin-Frontend`, base `main`).
